...
func extractManifestIncludes(value any, manifestPath string) ([]string, []string) {
	...
	for _, include := range rawIncludes {
		if !isSupportedManifestIncludePath(include) {
			warnings = append(warnings, fmt.Sprintf("Ignoring includes entry %q in %s: use workflow files (workflows/, agentic-workflows/, %s), skill directories (skills/, %sskills/), or agent markdown files (agents/, %sagents/)", include, manifestPath, constants.WorkflowsDir, constants.GithubDir, constants.GithubDir))
			continue
		}
...
func extractManifestFiles(value any, manifestPath string) ([]string, []string) {
	...
	for _, file := range rawFiles {
		if !isSupportedPackageInstallablePath(file) {
			warnings = append(warnings, fmt.Sprintf("Ignoring files entry %q in %s: supported files are markdown (.md) files under workflows/, agentic-workflows/, or %s, or action workflow (.yml) files under %s", file, manifestPath, constants.WorkflowsDir, constants.WorkflowsDir))
			continue
		}
...
func isSupportedPackageInstallablePath(p string) bool {
	...
	if isActionWorkflowPath(cleaned) {
		...
		// Reject nested subdirectories: only direct children of the workflows directory are allowed.
