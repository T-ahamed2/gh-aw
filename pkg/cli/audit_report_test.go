...
func TestExtractPreAgentStepErrors(t *testing.T) {
	...
	t.Run("prioritizes error annotations over last step fallback", func(t *testing.T) {
		...
		require.Len(t, errors, 1, "Should return one error for the step with error annotations")
		assert.Equal(t, "activation/Generate agentic run info", errors[0].File, "Should reference the step with error annotations, not Complete job")
	...
	t.Run("returns error annotations from multiple steps", func(t *testing.T) {
		...
		// All returned errors should be from steps with error annotations, not the cleanup step
