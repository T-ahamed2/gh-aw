...
func TestExtractPreAgentStepErrors(t *testing.T) {
	...
	t.Run("prioritizes double-hash-error annotations over last step fallback", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "activation")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("returns double-hash-error annotations from multiple steps", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "agent")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("falls back to last step when no double-hash-error annotations exist", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "agent")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("extracts double-hash-error from flat job log files", func(t *testing.T) {
	...
	t.Run("falls back to last flat job log when no double-hash-error in flat files", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
