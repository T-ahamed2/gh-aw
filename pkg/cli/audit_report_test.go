...
func TestExtractPreAgentStepErrors(t *testing.T) {
	...
	t.Run("prioritizes error_marker annotations over last step fallback", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "activation")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("returns error_marker annotations from multiple steps", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "agent")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("falls back to last step when no error_marker annotations exist", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "agent")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("extracts error_marker from flat job log files", func(t *testing.T) {
	...
	t.Run("falls back to last flat job log when no error_marker in flat files", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
