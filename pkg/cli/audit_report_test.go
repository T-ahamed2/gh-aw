...
func TestExtractPreAgentStepErrors(t *testing.T) {
	...
	t.Run("prioritizes_error_annotations_over_last_step_fallback", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "activation")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("returns_error_annotations_from_multiple_steps", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "agent")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
		...
		// Higher-numbered step with no error marker
		...
		// All returned errors should be from steps with error marker, not the cleanup step
		for _, e := range errors {
			assert.NotEqual(t, "agent/Complete job", e.File, "Should not include cleanup step in errors")
		}
	})

	t.Run("falls_back_to_last_step_when_no_error_annotations_exist", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs", "agent")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
	...
	t.Run("extracts_error_from_flat_job_log_files", func(t *testing.T) {
		...
		assert.Contains(t, errors[0].Message, "Lockdown mode is enabled", "Message should contain the error annotation content")
		assert.NotContains(t, errors[0].Message, "2026-02-23T", "Should strip GHA timestamps")
	})

	t.Run("falls_back_to_last_flat_job_log_when_no_error_in_flat_files", func(t *testing.T) {
		dir := testutil.TempDir(t, "audit-step-*")
		workflowLogsDir := filepath.Join(dir, "workflow-logs")
		require.NoError(t, os.MkdirAll(workflowLogsDir, 0755))
