...
func FindClosestMatches(target string, candidates []string, maxResults int) []string {
	...
	for _, candidate := range candidates {
		...
		// Optimization: skip Levenshtein if length difference exceeds maxDistance
...
func LevenshteinDistance(a, b string) int {
	...
	// Ensure b is the shorter string to minimize the DP table size
	if aLen < bLen {
		a, b = b, a
		aLen, bLen = bLen, aLen
	}
	...
	var row []int
	var buf [65]int // +1 for the 0th element, handles strings up to 64 bytes
	if bLen+1 <= len(buf) {
		row = buf[:bLen+1]
	} else {
		row = make([]int, bLen+1)
