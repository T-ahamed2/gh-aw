...
func TestReadFileFromHEAD(t *testing.T) {
	...
	t.Run("returns error when relative path starts with a hyphen", func(t *testing.T) {
		...
		assert.Contains(t, err.Error(), "should not start with a hyphen", "error should mention hyphen limitation")
	})
}
...
func TestValidateGitArg(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "hyphenated names allowed if not prefix",
			input:       "some-hyphenated-name",
			expectError: false,
		},
	}
