...
func (r *ActionResolver) resolveFromGitHub(ctx context.Context, repo, version string) (string, error) {
	...
	for depth := 0; objType == "tag"; depth++ {
		...
		err = func() error {
			...
			if peelErr != nil {
				return fmt.Errorf("failed to peel annotated tag %s@%s: %w; should verify that tag exists and is valid", repo, version, peelErr)
			}
			sha, objType, err = ParseTagRefTSV(string(output2))
			if err != nil {
				return fmt.Errorf("failed to parse peeled tag API response for %s@%s: %w; should verify API response is valid", repo, version, err)
			}
			return nil
		}()
		if err != nil {
