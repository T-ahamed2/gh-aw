...
func (r *ActionResolver) resolveFromGitHub(ctx context.Context, repo, version string) (string, error) {
	...
	for depth := 0; objType == "tag"; depth++ {
		peelErr := func() error {
			...
			if execErr != nil {
				return fmt.Errorf("failed to peel annotated tag %s@%s: %w", repo, version, execErr)
			}
			var parseErr error
			sha, objType, parseErr = ParseTagRefTSV(string(output2))
			if parseErr != nil {
				return fmt.Errorf("failed to parse peeled tag API response for %s@%s: %w", repo, version, parseErr)
			}
			return nil
		}()
		if peelErr != nil {
