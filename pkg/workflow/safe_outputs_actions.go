...
func fetchRemoteActionYAML(repo, subdir, ref string) (*actionYAMLFile, error) {
	for _, filename := range []string{"action.yml", "action.yaml"} {
		...
		// Wrap context operation in an anonymous function so defer cancel() executes
		// at the end of each iteration to prevent resource leaks.
		output, err := func() ([]byte, error) {
			ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
			defer cancel()
			return ExecGHContext(ctx, "api", apiPath, "--jq", ".content").Output()
		}()
