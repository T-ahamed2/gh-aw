...
func fetchRemoteActionYAML(repo, subdir, ref string) (*actionYAMLFile, error) {
	for _, filename := range []string{"action.yml", "action.yaml"} {
		actionYAML, err := func() (*actionYAMLFile, error) {
			...
			if err != nil {
				return nil, fmt.Errorf("failed to fetch %s from %s@%s: %w", filename, repo, ref, err)
			}
			...
			if decErr != nil {
				return nil, fmt.Errorf("failed to decode content for %s: %w", contentPath, decErr)
			}
		...
		if err != nil {
			safeOutputActionsLog.Printf("Warning: %v", err)
			continue
		}
