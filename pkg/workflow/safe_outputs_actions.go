...
func fetchRemoteActionYAML(repo, subdir, ref string) (*actionYAMLFile, error) {
	...
	return nil, fmt.Errorf("retrieve remote action YAML from %s@%s (subdir=%q): expected action.yml or action.yaml to exist; should check repository path configuration", repo, ref, subdir)
}
