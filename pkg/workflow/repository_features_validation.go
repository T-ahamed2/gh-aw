...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error to allow compilation in environments where issues are disabled (e.g. forks)
...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", NewOperationError("get", "current repository", "", err,
			"Ensure you are running in a Git repository with a configured GitHub remote; Example: git remote add origin https://github.com/owner/repo")
	}
	...
	if repo.Owner == "" || repo.Name == "" {
		return "", NewValidationError("repository", fmt.Sprintf("owner: %q, name: %q", repo.Owner, repo.Name),
			...
			"Check your Git remote configuration; Example: git remote set-url origin https://github.com/owner/repo")
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if err != nil {
		return nil, NewOperationError("check", "discussions status", repo, err,
			"Ensure the GitHub CLI is authenticated and has permission to read repository settings; Example: gh auth refresh -s repo")
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, NewOperationError("query", "discussions status via GraphQL", repo, err,
			"Verify your network connection and GitHub CLI authentication; Example: gh auth status")
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, NewOperationError("parse", "GraphQL response", repo, err,
			"Check for GitHub API service disruptions or unexpected response formats; Example: https://www.githubstatus.com/")
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, NewOperationError("create", "GitHub REST client", "", err,
			"Verify your environment is correctly configured for the GitHub CLI; Example: export GH_TOKEN=your_token")
	}
	...
	if err != nil {
		return false, NewOperationError("query", "repository via REST", repo, err,
			"Ensure the repository exists and your token has the 'repo' scope; Example: gh auth refresh -s repo")
	}
