...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error to allow compilation in forks without issues enabled.
			// Strategy: Similar to discussions, we will attempt to create the issue at runtime.
