...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error to allow compilation in forks and environments where
			// issue status cannot be reliably determined. The runtime create_issue handler will
			// provide specific failure details if creation is attempted and fails.
			warningMsg := fmt.Sprintf("Repository %s may not have issues enabled. The workflow uses safe-outputs.create-issue, which will attempt to create issues at runtime. If creation fails, ensure issues are enabled in repository settings.", repo)
			repositoryFeaturesLog.Printf("Warning: %s", warningMsg)
			if c.verbose {
				fmt.Fprintln(os.Stderr, console.FormatWarningMessage(warningMsg))
			}
