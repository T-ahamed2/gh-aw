...
func TestActionResolverFailedResolutionCache(t *testing.T) {
	...
	if _, exists := resolver.GetUsedCacheKeys()[cacheKey]; !exists {
	...
	if err2 == nil || !strings.Contains(err2.Error(), expectedErrMsg) {
...
func TestActionResolverUsedCacheKeysOnCacheHit(t *testing.T) {
	...
	if _, exists := usedKeys["owner/action-a@v1"]; !exists {
		t.Error("Expected owner/action-a@v1 to be in used cache keys after a cache hit")
	}
	if _, exists := usedKeys["owner/action-b@v2"]; exists {
...
func TestActionResolverGetUsedCacheKeysReturnsCopy(t *testing.T) {
	...
	if _, exists := resolver.GetUsedCacheKeys()["owner/action-a@v1"]; !exists {
