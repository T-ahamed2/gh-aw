...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error per fork issues disabled behavior
...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("retrieve current repository: %w; should check git remote configuration", err)
	}

	// Validate that owner and name are not empty
	if repo.Owner == "" || repo.Name == "" {
		return "", fmt.Errorf("repository owner or name should be non-empty (owner: %q, name: %q)", repo.Owner, repo.Name)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("verify discussions enablement status: %w; should check connection settings", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("verify issues enablement status: %w; should check connection settings", err)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("query discussions enablement status: %w; should verify connection", err)
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, fmt.Errorf("parse discussions GraphQL response: %w; expected valid format", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("initialize GitHub REST client: %w; should check authentication token", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("fetch repository details: %w; should verify valid settings", err)
	}
