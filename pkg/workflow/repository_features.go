...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error to avoid blocking E2E recompile or CI jobs on forked repositories where issues are disabled.
...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("retrieve current repository: %w; should check if git is initialized and remote configuration is valid", err)
	}

	// Validate that owner and name are not empty
	if repo.Owner == "" || repo.Name == "" {
		return "", fmt.Errorf("retrieve current repository: owner %q or name %q should be non-empty; should verify remote URL format (example: 'https://github.com/owner/repo')", repo.Owner, repo.Name)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if cached, exists := repositoryFeaturesCache.Load(repo); exists {
		...
		if !ok {
			...
			return nil, fmt.Errorf("resolve repository feature cache: cache entry for %s should be *RepositoryFeatures but different type was found; type: %T", repo, cached)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("query repository discussions state: %w; should check network connection and verify that required scopes are valid", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("query repository issues state: %w; should check network connection and verify that required scopes are valid", err)
	}
	...
	if !ok {
		repositoryFeaturesCache.Delete(repo)
		return nil, fmt.Errorf("resolve repository feature cache: cache entry for %s should be *RepositoryFeatures but different type was found; type: %T", repo, actual)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("retrieve discussions status via GraphQL API: %w; should verify that the GH_TOKEN is valid and has expected permissions", err)
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, fmt.Errorf("decode GraphQL response format: %w; should expect valid JSON from GitHub API", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("initialize REST API client: %w; should verify that configuration is valid", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("query repository status via REST API: %w; should verify network connection and make sure repository exists", err)
	}
