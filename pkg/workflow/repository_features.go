...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("retrieve current repository: %w; should check git remote and workspace status", err)
	}

	// Validate that owner and name are not empty
	if repo.Owner == "" || repo.Name == "" {
		return "", fmt.Errorf("repository owner and name should be non-empty (owner: %q, name: %q)", repo.Owner, repo.Name)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if cached, exists := repositoryFeaturesCache.Load(repo); exists {
		...
		if !ok {
			...
			return nil, fmt.Errorf("invalid repository feature cache entry for %s: expected *RepositoryFeatures, got %T; should delete cached entry", repo, cached)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("determine repository discussions enablement: %w; requires valid API authentication", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("determine repository issues enablement: %w; requires valid API authentication", err)
	}
	...
	if !ok {
		repositoryFeaturesCache.Delete(repo)
		return nil, fmt.Errorf("invalid repository feature cache entry for %s: expected *RepositoryFeatures, got %T; should delete cached entry", repo, actual)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("query discussions status via GraphQL: %w; should verify network and API token", err)
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, fmt.Errorf("parse discussions GraphQL API response: %w; should expect valid JSON format", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("create REST client: %w; should verify network and credentials", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("query repository details via REST API: %w; should verify network and credentials", err)
	}
