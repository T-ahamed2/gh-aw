...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error to avoid blocking E2E recompile/CI jobs on forked repositories where issues are disabled
...
func getCurrentRepositoryUncached() (string, error) {
	...
	if repo.Owner == "" || repo.Name == "" {
		return "", fmt.Errorf("repository owner or name is empty (owner: %q, name: %q); expected a valid GitHub repository", repo.Owner, repo.Name)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("query repository discussions: %w; should check GitHub token permissions and network connectivity", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("query repository issues: %w; should check GitHub token permissions and network connectivity", err)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("execute GraphQL API query for discussions: %w; should check GitHub CLI authentication status", err)
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, fmt.Errorf("parse GraphQL API response: %w; should check response payload structure", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("initialize REST API client: %w; should check authentication credentials", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("fetch repository info from REST API: %w; should check repository accessibility", err)
	}
