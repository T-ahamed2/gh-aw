...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("retrieve current repository: %w; should ensure git or GH_REPO is valid", err)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("verify discussions capability: %w; should ensure network/auth is valid", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("verify issues capability: %w; should ensure network/auth is valid", err)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("query discussions status: %w; should ensure GraphQL API or token is valid", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("initialize REST client: %w; should ensure GitHub environment is valid", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("query repository REST metadata: %w; should check connection/token is valid", err)
	}
