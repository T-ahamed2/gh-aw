...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error to avoid blocking forked repositories where issues are disabled
...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("retrieve current repository: %w; should check if git is initialized and has a remote configuration", err)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("retrieve discussions status: %w; should check if the github API is accessible and token has read scope", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("retrieve issues status: %w; should check if the github API is accessible and token has read scope", err)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("retrieve discussions status from GraphQL API: %w; should check if query parameters are valid and network is connected", err)
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, fmt.Errorf("parse GraphQL response: %w; should check if response body matches GraphQL schema", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("initialize REST client: %w; should check if github environment variables are configured correctly", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("retrieve repository metadata over REST API: %w; should check if network is online and token is valid", err)
	}
