...
func (c *Compiler) validateRepositoryFeatures(workflowData *WorkflowData) error {
	...
	if workflowData.SafeOutputs.CreateIssues != nil {
		...
		if err != nil {
			...
			// Changed to warning instead of error per issue feedback to avoid blocking fork compilations
			warningMsg := fmt.Sprintf("Repository %s may not have issues enabled. The workflow will attempt to create issues at runtime. If creation fails, enable issues in repository settings or remove create-issue from safe-outputs.", repo)
			repositoryFeaturesLog.Printf("Warning: %s", warningMsg)
			if c.verbose {
				fmt.Fprintln(os.Stderr, console.FormatWarningMessage(warningMsg))
			}
...
func getCurrentRepositoryUncached() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("retrieve current repository: %w; should verify git remote configuration", err)
	}

	// Validate that owner and name are not empty
	if repo.Owner == "" || repo.Name == "" {
		return "", fmt.Errorf("retrieve current repository: owner %q or name %q is empty; should verify remote URL format", repo.Owner, repo.Name)
	}
...
func getRepositoryFeatures(repo string, verbose bool) (*RepositoryFeatures, error) {
	...
	if cached, exists := repositoryFeaturesCache.Load(repo); exists {
		...
		if !ok {
			...
			return nil, fmt.Errorf("repository feature cache entry for %s expects *RepositoryFeatures, got %T; should check cache corruption", repo, cached)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("query discussions enablement status: %w; should check network or token scope", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("query issues enablement status: %w; should check network or token scope", err)
	}
	...
	if !ok {
		repositoryFeaturesCache.Delete(repo)
		return nil, fmt.Errorf("repository feature cache entry for %s expects *RepositoryFeatures, got %T; should check cache corruption", repo, actual)
	}
...
func checkRepositoryHasDiscussionsUncached(repo string) (bool, error) {
	...
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		return false, fmt.Errorf("repository format for %s requires owner/repo structure (example: github/gh-aw)", repo)
	}
	...
	if err != nil {
		return false, fmt.Errorf("execute GraphQL query for discussions status: %w; should verify API credentials", err)
	}

	var response GraphQLResponse
	if err := json.Unmarshal(stdOut.Bytes(), &response); err != nil {
		return false, fmt.Errorf("decode GraphQL response payload: %w; should ensure server returned expected format", err)
	}
...
func checkRepositoryHasIssuesUncached(repo string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("initialize REST API client: %w; should verify authentication settings", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("fetch repository features metadata: %w; should verify repository URL and permissions", err)
	}
